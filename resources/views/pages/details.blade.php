...
<head>
  ...
  <style>
    ...
    .nav-link { color: var(--dark); transition: 0.3s; }
    ...
    .sidebar-link { display: flex; align-items: center; padding: 14px 18px; color: var(--dark); text-decoration: none; font-weight: 600; border-radius: 12px; margin-bottom: 8px; }
    .sidebar-link i { width: 24px; margin-right: 12px; color: var(--primary); }
    .sidebar-link:hover { background: #f1f5f9; }

    /* Page Content */
    .page-title h1 { font-weight: 800; background: var(--gradient); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    ...
    .res-card__media { width: 100%; height: 550px; object-fit: cover; cursor: pointer; transition: 0.8s; }
    ...
    .res-meta span { background: #f1f5f9; padding: 8px 16px; border-radius: 12px; font-weight: 600; font-size: 0.9rem; display: inline-block; margin-bottom: 8px; margin-right: 4px; }
    ...
    .btn-reserver { background: var(--gradient) !important; border: none !important; border-radius: 16px !important; padding: 18px !important; font-weight: 800 !important; color: white !important; box-shadow: 0 10px 15px -3px rgba(0, 175, 185, 0.4); transition: 0.3s; }
  ...
  <aside id="sidebar">
    ...
    <div class="sidebar-menu">
      @if(Auth::check() && Auth::user()->type_compte === 'professionnel')
        <a class="sidebar-link" href="{{ route('pro.dashboard') }}"><i class="fas fa-chart-line"></i> Dashboard</a>
        <a class="sidebar-link" href="{{ route('mes_demandes') }}"><i class="fas fa-envelope"></i> Demandes</a>
        <a class="sidebar-link" href="{{ route('reservationRecu') }}"><i class="fas fa-calendar-check"></i> Réservations</a>
  ...
  <main class="container py-5">
    ...
    <article class="res-card">
      ...
      <div class="res-card__visual">
        <img id="mainPreview" class="res-card__media" src="{{ $first }}" alt="{{ $residences_details->nom }}" role="button">
        @if(count($images) > 1)
      ...
      <div class="res-details">
        ...
        <h4 class="fw-bold mt-4 mb-3">Description</h4>
  ...
  <script>
    ...
    (function(){
      ...
      function validate() {
        ...
        if(start < today) {
          alertBox.textContent = "La date d'arrivée ne peut pas être dans le passé.";
      ...
      [d1, d2].forEach(el => el.addEventListener('change', validate));

      form.addEventListener('submit', (ev) => {
        ev.preventDefault();
        const n = validate();
        document.getElementById('confirmationMessage').innerHTML = `Vous réservez pour <strong>${n} nuits</strong>.<br>Total : <strong>${(n * prix).toLocaleString('fr-FR')} FCFA</strong>`;
